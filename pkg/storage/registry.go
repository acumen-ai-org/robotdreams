...
// Constructor builds a StorageBackend from a parsed URI. Backend packages
// implement one and register it against a URI scheme via Register, so that
// callers can select a backend at runtime by URI (e.g. "file:///var/data")
...
// backend package (e.g. `import _ "github.com/acumen-ai-org/robotdreams/pkg/storage/localfs"`)
// is enough to make the scheme available to New.
//
...
// registered for its scheme. Callers must blank-import the desired backend
// package(s) so their init() functions run and register a Constructor
